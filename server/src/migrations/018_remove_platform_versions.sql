-- 018: v3.1 — remove platform version selection from the offer.
--
-- Owner decision (2026-08-26): the Website / Mobile App / Website + Mobile
-- App choice is gone. Pricing is now derived from a single base (PHP 20,000)
-- with tier multipliers; templates no longer store an individual price.
--
-- Data preservation rules:
--   1. Existing `intake_template_selections.project_version` values
--      ('desktop' | 'mobile' | 'both') are kept exactly as stored.
--   2. New selections insert NULL. The column becomes nullable; nothing is
--      backfilled and no historical value is rewritten or defaulted.
--   3. Re-saving an old draft must not null out its stored version, so the
--      template upsert below preserves any existing non-NULL value and only
--      writes EXCLUDED when the row has none.

ALTER TABLE public.intake_template_selections
  ALTER COLUMN project_version DROP NOT NULL;
...
BEGIN
  ...
  IF NULLIF(p_template, '') IS NOT NULL THEN
    ...
    ON CONFLICT (intake_id) DO UPDATE SET
      ...
      -- v3.1: never overwrite a stored platform version with NULL. A value
      -- captured before the removal survives every later save of the intake.
      project_version = coalesce(public.intake_template_selections.project_version, EXCLUDED.project_version),

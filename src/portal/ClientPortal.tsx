...
const NAV_ITEMS: { id: PortalView; label: string; glyph: string }[] = [
  { id: "overview", label: "Overview", glyph: "⌂" },
  { id: "projects", label: "Projects", glyph: "▱" },
  { id: "settings", label: "Preferences", glyph: "⚙" },
]
...
function Glyph({ children }: { children: ReactNode }) {
  return <span className="portal-glyph" aria-hidden="true">{children}</span>
}
...
function AuthScreen({ onEnter }: { onEnter: () => void }) {
    ...
    return (
      <main className="portal-auth-shell">
        <div className="portal-auth-card">
          <PortalLogo />
          <div className="portal-auth-icon portal-auth-icon-warm">↻</div>
  ...
  return (
    <main className="portal-auth-shell">
      <div className="portal-auth-card">
        <PortalLogo />
        <div className="portal-auth-icon">↗</div>
        ...
        {sent ? (
          <div className="portal-sent-message" role="status">
            <span className="portal-sent-check">✓</span>
          ...
          <form onSubmit={(event) => { event.preventDefault(); setSent(true) }}>
            ...
            <button className="portal-button portal-button-primary portal-button-wide" type="submit">Email me a sign-in link <span>→</span></button>
          </form>
        )}
        ...
        {sent && <button className="portal-demo-link" onClick={onEnter}>Continue to demo portal →</button>}
      </div>
    </main>
  )
}
...
function EmptyProjects({ onStart }: { onStart: () => void }) {
  return (
    <div className="portal-empty-state">
      ...
      <button className="portal-button portal-button-secondary" onClick={onStart}>Start an intake <span>→</span></button>
    </div>
  )
}
  ...
  return (
    <>
      <section className="portal-welcome-row">
        ...
        <button className="portal-button portal-button-quiet"><span className="portal-button-glyph">?</span> Need a hand?</button>
      </section>

      <section className="portal-stat-grid" aria-label="Project summary">
        <div className="portal-stat-card"><span className="portal-stat-label">ACTIVE PROJECT</span><strong>{project.name}</strong><span className="portal-stat-link" onClick={onOpenProject}>View project <span>↗</span></span></div>
      ...
      <section className="portal-section-heading"><div><p className="portal-eyebrow">IN FOCUS</p><h2>Northstar membership platform</h2></div><button className="portal-link-button" onClick={onOpenProject}>Open project workspace <span>→</span></button></section>
      ...
      <section className="portal-featured-card">
        ...
        <div className="portal-featured-bottom"><span>Submitted {project.submitted}</span><button className="portal-button portal-button-primary" onClick={onOpenProject}>View project <span>→</span></button></div>
      </section>

      <section className="portal-lower-grid">
        <div className="portal-panel portal-timeline-panel"><div className="portal-panel-heading"><div><p className="portal-eyebrow">YOUR JOURNEY</p><h2>Project timeline</h2></div><span className="portal-panel-count">2 of 4</span></div><div className="portal-timeline">{TIMELINE.map((item) => <div className={`portal-timeline-item ${item.done ? "is-done" : ""} ${item.current ? "is-current" : ""}`} key={item.label}><div className="portal-timeline-marker">{item.done ? "✓" : ""}</div><div className="portal-timeline-copy"><div><strong>{item.label}</strong><span>{item.date}</span></div><p>{item.detail}</p></div></div>)}</div></div>
        <div className="portal-panel portal-next-panel"><div className="portal-panel-heading"><div><p className="portal-eyebrow">KEEP THINGS MOVING</p><h2>Next steps</h2></div><Glyph>✦</Glyph></div><div className="portal-task-list"><button className="portal-task portal-task-active" onClick={onOpenProject}><span className="portal-task-icon">↑</span><span><strong>Share remaining assets</strong><small>{outstanding.length} item{outstanding.length === 1 ? "" : "s"} still needed</small></span><span className="portal-task-arrow">→</span></button><button className="portal-task" onClick={onOpenProject}><span className="portal-task-icon portal-task-icon-soft">◌</span><span><strong>Review your proposal</strong><small>Everything in one clear summary</small></span><span className="portal-task-arrow">→</span></button><div className="portal-trust-note"><span>✧</span><p>We’ll always tell you what’s happening next — no guesswork, no jargon.</p></div></div></div>
      </section>

      <section className="portal-recent-heading"><div><p className="portal-eyebrow">ALL PROJECTS</p><h2>Project history</h2></div><button className="portal-link-button" onClick={onOpenProject}>See all projects <span>→</span></button></section>
      <div className="portal-project-mini-grid">{PROJECTS.slice(1).map((item) => <button className="portal-project-mini" key={item.id} onClick={() => onOpenNamedProject(item.id)}><div><span className="portal-reference">{item.reference}</span><h3>{item.name}</h3><p>{item.type}</p></div><div><StatusPill status={item.status} tone={item.statusTone} /><span className="portal-mini-arrow">↗</span></div></button>)}</div>
    </>
  )
}

function Projects({ onOpenProject }: { onOpenProject: (id: string) => void }) {
  return <><section className="portal-welcome-row"><div><p className="portal-eyebrow">YOUR WORKSPACE</p><h1>All projects</h1><p className="portal-welcome-copy">A clear view of everything we’re building together.</p></div><button className="portal-button portal-button-secondary">＋ New project</button></section><div className="portal-project-grid">{PROJECTS.map((project) => <button className="portal-project-card" key={project.id} onClick={() => onOpenProject(project.id)}><div className="portal-project-card-top"><span className="portal-reference">{project.reference}</span><span className="portal-card-arrow">↗</span></div><h2>{project.name}</h2><p>{project.description}</p><div className="portal-project-card-meta"><span>{project.type}</span><StatusPill status={project.status} tone={project.statusTone} /></div><div className="portal-mini-progress"><span style={{ width: `${project.progress}%` }} /></div><div className="portal-project-card-foot"><span>{project.progress}% complete</span><span>Updated {project.updated}</span></div></button>)}</div></>
}
...
function AssetsPanel({ project, onUpload }: { project: Project; onUpload: (file: File) => void }) {
  ...
  return <div className="portal-assets-panel"><div className="portal-detail-heading"><div><p className="portal-eyebrow">ASSET COMPLETION</p><h2>Bring your project to life.</h2><p>Share the remaining files our team needs. Every upload is checked before it’s added to your project.</p></div><div className="portal-asset-score"><strong>{project.assets.length - requiredCount}/{project.assets.length}</strong><span>ready</span></div></div><label className={`portal-upload-zone ${dragging ? "is-dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); const file = event.dataTransfer.files[0]; if (file) onUpload(file) }}><input type="file" onChange={handleFile} accept="image/png,image/jpeg,application/pdf,image/svg+xml" /><span className="portal-upload-icon">↑</span><strong>Drop a file here, or <em>browse</em></strong><small>PNG, JPG, SVG, or PDF · up to 25 MB</small></label><div className="portal-asset-list">{project.assets.map((asset) => <div className="portal-asset-row" key={asset.id}><div className="portal-file-icon">{asset.name.toLowerCase().includes("logo") ? "◇" : "▤"}</div><div className="portal-asset-copy"><strong>{asset.name}</strong><span>{asset.meta}</span></div><span className={`portal-asset-status portal-asset-status-${asset.status.replace(" ", "-")}`}>{asset.status === "accepted" ? "✓ Accepted" : asset.status === "pending scan" ? "◌ Pending scan" : "Required"}</span><button className="portal-asset-more" aria-label={`More options for ${asset.name}`}>···</button></div>)}</div><div className="portal-asset-footnote"><span>⌁</span> Files are private to your project and scanned for safety before review.</div></div>
}

function AgreementPanel() {
  return <div className="portal-agreement-panel"><div className="portal-agreement-illustration">◫</div><p className="portal-eyebrow">AGREEMENT</p><h2>Your project agreement will appear here.</h2><p>Once the final scope is ready, you’ll be able to review the details in one calm, readable place. We’ll let you know when it’s ready.</p><div className="portal-agreement-seam"><span>✧</span><div><strong>Nothing to sign just yet</strong><small>This space is reserved for your final agreement.</small></div></div></div>
}
...
function ProjectDetail({ project, onBack }: { project: Project; onBack: () => void }) {
  ...
  return <><button className="portal-back-button" onClick={onBack}>← <span>Back to overview</span></button><section className="portal-detail-top"><div><span className="portal-reference">{projectState.reference} · {projectState.type}</span><h1>{projectState.name}</h1><p>{projectState.description}</p></div><StatusPill status={projectState.status} tone={projectState.statusTone} /></section><nav className="portal-detail-tabs" aria-label="Project sections">{tabs.map((item) => <button className={tab === item.id ? "is-active" : ""} key={item.id} onClick={() => setTab(item.id)}>{item.label}{item.id === "assets" && projectState.assets.some((asset) => asset.status === "required") && <span className="portal-tab-dot" />}</button>)}</nav>{tab === "summary" && <div className="portal-detail-grid"><div className="portal-panel portal-summary-panel"><div className="portal-panel-heading"><div><p className="portal-eyebrow">PROJECT BRIEF</p><h2>What we’re making</h2></div></div><p className="portal-summary-copy">{projectState.description} The first release is designed to give members an easy way to find what matters, stay connected, and keep their progress moving.</p><div className="portal-scope-block"><span className="portal-eyebrow">PRIORITIES</span><div className="portal-chip-row"><span>Member experience</span><span>Simple navigation</span><span>Responsive design</span><span>Secure payments</span></div></div><div className="portal-summary-details"><div><span>PROJECT TYPE</span><strong>Custom web app</strong></div><div><span>SUBMITTED</span><strong>{projectState.submitted}</strong></div><div><span>LAST UPDATED</span><strong>{projectState.updated}</strong></div></div></div><div className="portal-panel portal-mini-timeline"><div className="portal-panel-heading"><div><p className="portal-eyebrow">WHAT’S NEXT</p><h2>Your timeline</h2></div></div>{TIMELINE.map((item) => <div className={`portal-mini-timeline-row ${item.done ? "is-done" : ""}`} key={item.label}><span className="portal-mini-marker">{item.done ? "✓" : ""}</span><div><strong>{item.label}</strong><span>{item.date}</span></div></div>)}</div></div>}{tab === "build-card" && <BuildCard project={projectState} />}{tab === "assets" && <AssetsPanel project={projectState} onUpload={upload} />}{tab === "agreement" && <AgreementPanel />}</>
}
...
export default function ClientPortal() {
  ...
  return <div className="portal-app-shell"><aside className="portal-sidebar"><PortalLogo /><div className="portal-sidebar-label">YOUR SPACE</div><nav className="portal-sidebar-nav" aria-label="Main navigation">{NAV_ITEMS.map((item) => <button className={view === item.id && !selectedProject ? "is-active" : ""} key={item.id} onClick={() => selectView(item.id)}><Glyph>{item.glyph}</Glyph>{item.label}{item.id === "projects" && <span className="portal-nav-count">3</span>}</button>)}</nav><div className="portal-sidebar-bottom"><div className="portal-sidebar-help"><span>✦</span><strong>Need a hand?</strong><p>We’re here when you need us.</p><button>Contact support <span>→</span></button></div><button className="portal-account-button" onClick={() => setAuthenticated(false)}><span className="portal-avatar portal-avatar-small">AR</span><span><strong>Alex Rivera</strong><small>Northstar</small></span><span className="portal-account-more">···</span></button></div></aside><main className="portal-main"><header className="portal-mobile-header"><button className="portal-mobile-menu" aria-label={mobileNavOpen ? "Close navigation" : "Open navigation"} aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen((open) => !open)}>☰</button><PortalLogo /><button className="portal-avatar portal-avatar-small" onClick={() => setAuthenticated(false)}>AR</button></header>{mobileNavOpen && <nav className="portal-mobile-nav" aria-label="Mobile navigation">{NAV_ITEMS.map((item) => <button className={view === item.id && !selectedProject ? "is-active" : ""} key={item.id} onClick={() => selectView(item.id)}><Glyph>{item.glyph}</Glyph>{item.label}</button>)}</nav>}<div className="portal-content">{content}</div><footer className="portal-footer"><span>© 2026 M-Thryve</span><span>Built with care for your next chapter <b>✦</b></span><button>Privacy</button></footer></main></div>
}
